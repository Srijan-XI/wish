function typeGreeting(text, element, speed = 60) {
  let i = 0;
  element.textContent = '';
  function type() {
    if (i < text.length) {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  typeGreeting('Happy Birthday!', greetingEl);
  launchBalloons();
...
document.getElementById('wishBtn').addEventListener('click', () => {
  ...
  const greetingEl = document.getElementById('greeting');
  const wish = name 
    ? `Happy Birthday, ${name}! 🎉 May your dreams come true!`
    : 'May all your wishes come true!';

  typeGreeting(wish, greetingEl);
...
function launchBalloons() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    bal.style.width = '36px';
    bal.style.height = '48px';
